...
## 17. 보드 배치 = 목표 덱의 레벨별 빌드업 · 상점 ★2 규칙 · 고정 덱 엄격 모드 · 성급 미상 — jev-strategist, 2026-09-25

사용자 규칙 네 건(같은 라운드, 커밋하지 않음). 모두 코드·통계 전용이다. Jev 호출·질문 수는 늘지 않는다(test.png q=25, live4 q=27 그대로).
1. "중요한 건 각 최종덱에 맞는 빌드업을 추천해야 한다는 거야." (§17.1~17.5)
2. "3성 유닛 덱이 아닌 이상, 이미 2성이면 추천하지 말아줘." (§17.6)
3. 성급 미상(vision이 `star=None`, `_workspace/35_unit_tracking.md`)을 ★1로 가정하지 않는다. (§17.7)
4. "목표덱을 내가 고정하면 그 덱에 적합한 기물과 아이템만 추천해줘야 해." (§17.8)

### 17.1 무엇이 바뀌나
예전(§6·§10·§11): 보유 유닛마다 여러 신호(목표 덱 소속 x `plan_comp_scale` — 2스테이지 0.3, s_now, 특성, 성급, 아이템, Early Comps 스테이지 보드 통계 x `plan_now_scale` 2.0)를 더해 탐욕으로 골랐다. 초반에는 목표 덱 빌드업보다 "지금 강한" 유닛과 다른 덱의 스테이지 보드가 이겼다.

지금: **목표 덱(1위, 고정 중이면 고정 덱)의 내 레벨 빌드업 보드**(`CompStats.buildup`, MetaTFT comp_details early_options/options — `stats/metatft_convert.py:216`)가 드라이버다.

| 단계 | 규칙 |
|---|---|
| 기준 보드 | 내 레벨 L의 빌드업 보드 중 **표본 상위 `ref_top_boards`(3)개**에서 보유(이름 확인) 유닛이 가장 많은 보드. 동률이면 games → avg_place. 유닛 수 < L − `ref_min_units_gap`(1)인 보드는 수집 잡음으로 뺀다(실데이터: 카직스 레벨 5 표본 1위가 [카직스] 1기 보드) |
| 레벨 없음 | L에 빌드업이 없으면 가장 가까운 **아래** 레벨. 아래도 없으면(레벨 3 등) 가장 낮은 레벨(보통 4) |
| 다음 목표 | 기준 레벨 위 첫 레벨. 그 레벨 상위 3개 중 (보유 ∪ 기준 보드)와 가장 겹치는 보드 |
| 1단계 | 기준 보드 보유 유닛을 **구조적으로 먼저** 올린다(가중치가 아니라 순서 — 임시 유닛이 점수로 끼어들 수 없다). 챔피언마다 가장 좋은 사본 1기 |
| 2단계 | 남는 칸만 임시 유닛: 기준 보드 특성(+ 목표 덱 핵심 특성) 기여 x `ref_trait_mult`(2.0) → 다음 레벨 빌드업 소속(`buildup` + `next_member` = 1.8) · 최종 보드 소속(스테이지 할인 없이) → 성급·아이템 → s_now·스테이지 보드 통계 x `ref_now_scale`(0.4, 보조) |
| 사본 | 성급 순위 ★1 < 미상 < ★2 < ★3(`star_rank`). 미상 가산 `star_unknown`(0.4) < 보드 유지 `keep`(0.5) → 보드의 ★1을 벤치의 미상 사본과 굳이 바꾸지 않는다. 알려진 ★2는 보드 ★1보다 먼저 |
| 교체 | 올릴 유닛은 기준 보드 유닛 먼저, 내릴 유닛은 점수 낮은 순(예전과 같음) |

2스테이지 뉘앙스: 기준 보드 유닛이 없을 때 빈 칸은 임시 유닛(★2 가산 1.5가 가장 크다)이 지킨다. 기준 보드 유닛을 하나라도 가지면 그 유닛이 ★2 무관 유닛보다 먼저다 — 예전 `test_board_plan_prefers_strong_now_early…`와 반대(그 테스트는 `follow_buildup=false`로 옮겨 예전 동작을 계속 고정한다).

Early Comps 스테이지 보드 통계(§11): 점수에는 보조(x0.4)로 남지만 **문구는 내지 않는다**("지금 이 스테이지 추천 보드 …", "다음 스테이지 …", "전환: …" 줄 제거). 다른 보드를 권해 기준 보드와 헷갈리기 때문이다. `BoardPlan.stage_board`·`transition` 계약 필드는 그대로 채운다(`transition.next_level/next_targets`는 기준 보드의 다음 레벨로).

`[board_plan] follow_buildup = false`이거나 목표 덱·빌드업이 없으면 예전 동작이다.

### 17.2 계약 추가(선택 필드, 기존 호환 — `contracts.py`)
```python
class BoardPlanEntry:  in_reference: bool | None = None       # 기준 보드 유닛인가(None = 기준 보드 없음)
class BoardPlan:
    level: int | None                 # 기준으로 삼은 내 레벨
    reference_level: int | None       # 기준 보드 레벨(내 레벨에 없으면 아래 레벨)
    reference_units: list[ChampionId]
    reference_games: int | None; reference_avg_place: float | None
    owned_in_reference: list[ChampionId]   # 이름 확인 보유 유닛만(추정 이름 0.75·이름 미상 제외)
    missing: list[ChampionId]              # 기준 보드 중 없는 유닛
    next_level: int | None; next_level_units: list[ChampionId]
class ItemAdvice:      note: str | None = None                 # §17.8
```
`BoardPlan.lineup`의 뜻(지금 보드에 둘 유닛)은 그대로다 — HUD 아이콘 줄(`hud_model.lineup_cells`)은 바뀌지 않고 동작한다.

### 17.3 표시(`app/report.py board_plan_lines`, 합쇼체) — 고정 크기 HUD에서 중요한 줄부터
```
기준 달빛 아펠리오스 니달리 · 칸 5 · 빈 칸 1
레벨 5 빌드업(달빛 아펠리오스 니달리): 알리스타 · 오른 · 쉔 · 바루스 · 자야 — 보유 2/5
보드: 알리스타(레벨 5 빌드업) · 오른(레벨 5 빌드업) · 피들스틱(임시 · 엄호대 1→2 활성 · …) · 렉사이(임시 · 싸움꾼 1→2 활성)
교체: ↑ 벤치에서 올리기 렝가(↔ 보드 알리스타 내리기) · 워윅(빈 칸)       ← 없으면 "교체: 없음(지금 배치를 유지하세요)"
상점에서 구하세요: 쉔 · 바루스 · 자야 (미확인 유닛 중에 있을 수 있습니다)   ← 미확인 칸이 있을 때만 괄호
레벨 6: +아칼리 (빌드업 6기)
판매: … / 판매 이유: … / 판매 참고: …                                    ← 기준 보드가 있으면 벤치 줄보다 먼저
벤치: …
참고: …
```
- 내 레벨에 빌드업이 없으면 기준 줄 끝에 "(레벨 6 빌드업 통계가 없어 레벨 5 기준입니다)".
- 성급 미상은 "★?"로 표시한다(`label`, 판매 줄 포함). 판매 합계에 미상이 있으면 "+N골드 이상".
- "교체:"로 시작하므로 오버레이 강조색(`overlay._plan_color`)은 그대로 적용된다.

### 17.4 판매·상점과의 일관성
- **판매**(`sell.py`): 후반(`late_from_stage` 전) 보호 경로에 `plan.reference_units`와 `plan.next_level_units`를 더했다. "상점에서 구하세요"의 유닛이나 기준 보드 유닛을 팔라고 하지 않는다.
- **상점**(`scoring.shop_advice`): 기준 보드에 있는데 아직 없는 유닛에 `[shop] reference_missing_bonus`(0.3)를 더하고 근거 앞에 "레벨 5 빌드업 부족"을 붙인다. 0.2로는 live4(2-5, 빈 칸 1)에서 자야가 0.46 [보류]로 남아 "상점에서 구하세요: … 자야"와 엇갈렸다. 기준 덱은 `Scorer.shop_target()`(표시 1위 = 고정 덱, `rescore_shop`은 직전 추천 1위). 기준 보드 계산은 `board_plan.buildup_reference`를 그대로 불러 보드 배치와 같은 보드를 쓴다.

### 17.5 전/후(`--screenshot … --jev mock --no-overlay`, 1920x1080 live4는 raw)
목표 덱 순위·근거는 두 화면 모두 **바뀌지 않았다**(diff 동일). 추천 지연 4~6ms(변화 없음).

**test.png**(2-6, 레벨 5, 1위 처형자 카직스, 보드 카시오페아·카밀·엘리스·아칼리·코그모, 벤치 확인 카밀·아칼리 + 미상 7):
```
전:  보드: 카시오페아(추천 스테이지 보드 · 통계: 2스테이지 1성 …) · 카밀(추천 스테이지 보드 · …) · 엘리스(악의 여단 2→3 활성 · …) · 아칼리 · 코그모
     참고: 지금 이 스테이지 추천 보드: 케이틀린·카밀·카시오페아·엘리스·라칸 (… 보유 3/5) — 케이틀린·라칸은(는) 상점에서 구하시면 됩니다
     참고: 다음 스테이지: 이 보드는 보통 +케이틀린·라칸 쪽으로 …   참고: 전환: … 레벨 6 목표 다이애나·헤카림·카직스·르블랑·오른
후:  레벨 5 빌드업(처형자 카직스): 다이애나 · 헤카림 · 카직스 · 르블랑 · 오른 — 보유 0/5
     보드: 카시오페아(임시) · 코그모(임시) · 아칼리(임시 · 적응가 1→2 활성) · 카밀(임시 · 약탈자 1→2 활성) · 엘리스(임시 · 악의 여단 2→3 활성)
     상점에서 구하세요: 다이애나 · 헤카림 · 카직스 · 르블랑 · 오른 (미확인 유닛 중에 있을 수 있습니다)
     레벨 6: +피들스틱 (빌드업 6기)
```
라인업은 같다(기준 보드 유닛을 하나도 갖지 않아 전부 임시). 달라진 점: 1위 덱과 무관한 "케이틀린·라칸을 구하라"(다른 덱의 스테이지 보드) 대신 카직스 덱의 레벨 5 빌드업과 부족 유닛을 보여 준다. 상점(자야·레오나·요릭 등)에는 카직스 빌드업 유닛이 없어 변화 없다.

**live4 준비.png**(2-5, 레벨 5, 1위 달빛 아펠리오스 니달리, 보드 오른·피들스틱·알리스타·렉사이, 벤치 전부 미상):
```
전:  보드: 알리스타(현 레벨 통계 상위 · 추천 스테이지 보드) · 렉사이(…) · 오른(…) · 피들스틱(…)   빈 칸 1
     참고: 지금 이 스테이지 추천 보드: 알리스타·르블랑·오른·렉사이·베이가 (… 보유 3/5) — 르블랑·베이가은(는) 상점에서 …   ← 베이가 덱 보드
     참고: 전환: 다리 알리스타·렉사이·오른 / 지금 전력용(교체 예정) 피들스틱 / 레벨 6 목표 아칼리·쉔·바루스·자야
     [상점] 1. [보류] 자야 0.26 · 빌드업
후:  레벨 5 빌드업(달빛 아펠리오스 니달리): 알리스타 · 오른 · 쉔 · 바루스 · 자야 — 보유 2/5
     보드: 알리스타(레벨 5 빌드업) · 오른(레벨 5 빌드업) · 피들스틱(임시 · …) · 렉사이(임시 · …)
     상점에서 구하세요: 쉔 · 바루스 · 자야 (미확인 유닛 중에 있을 수 있습니다)
     레벨 6: +아칼리 (빌드업 6기)
     [상점] 1. [구매] 자야 0.56 · 빌드업 — 레벨 5 빌드업 부족 · …
```
기준 보드는 레벨 5 상위 3개([아칼리·오른·쉔·바루스·자야] 405판, [알리스타·오른·쉔·바루스·자야] 401판, [아칼리·카밀·오른·쉔·바루스] 174판) 중 보유 2기(알리스타·오른)와 맞는 두 번째다.

### 17.6 상점 ★2 규칙(`Scorer.owned_star2_rule`, `[shop] skip_owned_star2 = true`)
- 대상: 이름·성급을 **확인한** 보유 유닛(신뢰도 ≥ `unit_min_conf` 0.8, star가 2 이상). 추정 이름(0.75)·성급 미상(None)에는 적용하지 않는다.
- 이미 ★2 → [보류], 근거 앞 "이미 2성 보유". 점수는 그대로 두고 구매만 막는다(골드 누적 계산에서도 빠진다).
- 예외: 기준 덱(1위 또는 고정 덱, `rescore_shop`은 직전 추천 1위) 최종 보드가 그 유닛을 ★3으로 원하면(`CompUnit.star >= 3`, HUD ★3 표시와 같은 원천) 평소 점수대로 두고 근거 앞 "3성 목표 · 보유 N/9"(★2 = 3기, ★3에 9기).
- 이미 ★3 → [보류] "이미 3성 보유".
- 단순하게 둔다: 1성 2 + 2성 2에서 사면 바로 ★3이 되는 경우도 ★3 목표 덱이 아니면 막는다(사용자 규칙 그대로).
- 전체 추천과 `rescore_shop` 모두 같은 함수(`shop_advice`)를 쓴다.

### 17.7 성급 미상(star None) — ★1로 가정하지 않는다
| 위치 | 예전 | 지금 |
|---|---|---|
| `features.copies_owned` | None → ★1 | "적어도 1기"(하한). `star_unknown(uid, units)` 추가 |
| `features.buy_makes_2star/3star` | None → ★1로 셈 | 확인된 ★1만 센다("사면 2성" 가산 없음) |
| `jev_state._unit_entry` | star None이면 키 삭제 | `"star": null`로 보낸다 |
| `jev_state` 상점 `copies_owned` | 전부 확인이면 정확값 | 성급 미상 사본이 있으면 `copies_owned_at_least` + `buy_makes_2star: "unknown"` |
| `candidates.unit_proxy` | None → ★1 | ★2 배수는 알려진 ★2+만(값은 1성과 같아 결과 불변) |
| `engine.resource_signature` | None → ★1(결과 같음) | 알려진 ★2+만 명시 |
| `board_plan` | `star or 1` | `star_rank`(★1 < 미상 < ★2), 표시 "★?", 스테이지 보드 매칭은 "적어도 ★1" |
| `sell` | None → ★1(쌍·판매가) | ★1 쌍·★2 판정에서 뺀다. 판매가는 ★1 최솟값 + 근거 "성급 미확인(판매가는 최소값)", 합계 "+N골드 이상". 라인업 사본 매칭은 미상끼리 |
| `scoring.owned_star2_rule` | — | 미상은 적용 안 함 |

`tests/fixtures/states/*.json` 유닛 66기는 전부 성급이 있어 fixture 추천은 변하지 않는다. `test_two_star_bonus_only_when_units_known`은 성급 없는 유닛 2기로 "사면 2성"을 기대했다 — 의도대로 `star: 1`을 명시하고, 미상 2기는 가산이 없음을 같은 테스트에 더했다.

### 17.8 고정 덱 엄격 모드(사용자 규칙, 고정하지 않으면 예전 그대로)
- **상점**: `Scorer.pinned_units()` = 고정 덱 최종 보드 + 캐리 + 빌드업 경로(내 레벨 L ~ L+2 레벨 보드 전부, L < 4면 4레벨부터). 밖의 유닛은 [보류], 근거 앞 "고정 덱에 없음". `[comp] pin_other_rel` 기본값 0.5 → **0.0**(다른 덱 경로 가중 없음). ★2/★3 사본은 고정 덱 유닛이면 그대로 산다(§17.6 적용).
- **아이템**: 고정 덱 캐리 BIS·핵심 아이템("top")만 권한다. 보조(다른 덱·범용)는 빼고, 초반 '지금 전력용'은 보유자가 보드 위의 고정 덱 유닛(최종 보드·빌드업 경로)일 때만. 하나도 없으면 `hold=True`, `ItemAdvice.note = "고정 덱 아이템을 만들 재료가 아직 없습니다 — 재료 보관"`(`item_lines`가 그 한 줄을 보여 준다). 고정 덱이 쓰지 않는 벤치 완성템은 보유자를 권하지 않는다. 근거 머리는 "고정 덱(…) 캐리/핵심 아이템".
- **판매**: `attach_sell(..., pinned=True)`(엔진이 `scorer.pinned`로 넘긴다) → 표시된 대안(2·3위) 덱 최종 보드를 지키지 않는다. 경로(최종·빌드업·기준 보드) 밖 유닛은 판매 후보다. 2스테이지 조용함 규칙(벤치 압박·이자·구매 자리일 때만)은 그대로다.
- **보드 배치**는 원래 고정 덱(`scorer.shown[0]`)을 따른다 — 고정을 바꾸면 기준 보드가 바뀐다(테스트).

### 17.9 설정
- `config/weights.toml [board_plan]`: `follow_buildup = true`, `ref_top_boards = 3`, `ref_min_units_gap = 1`, `ref_trait_mult = 2.0`, `next_member = 1.0`, `ref_now_scale = 0.4`, `star_unknown = 0.4` (`config.BoardPlanWeights`)
- `[shop]`: `reference_missing_bonus = 0.3`, `skip_owned_star2 = true` (`config.ShopWeights`)
- `[comp]`: `pin_other_rel = 0.0`

### 17.10 테스트
- `tests/advisor/test_buildup_reference.py` 신규(16개)
  - 기준 보드 고르기: 보유 0 → 표본 1위, 보유와 맞는 3위 보드, 상위 3 밖 보드는 더 맞아도 안 고름, 1기 잡음 보드 제외
  - 아래 레벨 폴백(레벨 6 없음 → 5, 다음 7) · 레벨 3 → 4 · 리포트 "통계가 없어 레벨 5 기준" 문구
  - follow_buildup 끔 / 목표 덱 없음 = 예전 동작
  - 부분 일치: 기준 보유 3기 먼저(`in_reference`, "레벨 5 빌드업"), 임시 2기("임시"), 교체 대상, missing, 리포트 줄(기준·교체 ↑·상점에서 구하세요·레벨 6), 전환 줄 없음, 합쇼체
  - 2스테이지: 기준 유닛(요릭) 먼저 + ★2 무관 유닛(알리스타)이 임시로 남음, 기준 줄 항상 표시
  - 임시 칸: 다음 레벨 빌드업 유닛(티모) > 무관 유닛(케넨)
  - 성급 미상: 보드 ★1 vs 벤치 미상 → 교체 없음 / 벤치 ★1 vs 미상 → 미상 / 벤치 ★2 > 보드 ★1
  - 이름 미상·추정 이름(0.75)은 보유로 세지 않음 + "(미확인 유닛 중에 있을 수 있습니다)"
  - 판매가 기준·다음 레벨 유닛을 팔라고 하지 않음
  - 고정 덱 전환(자이라 ↔ 달빛) → 기준 보드가 그 덱의 `buildup_reference`
  - 상점 ★2: 비리롤 ★2 → [보류] "이미 2성 보유"(전체·rescore 둘 다), ★3 목표(카직스) → "3성 목표 · 보유 3/9", ★3 보유 → "이미 3성 보유", 추정 이름·성급 미상·설정 끔 → 미적용, 규칙이 실제 [구매]를 막음(점수 동일)
  - 기준 보드 부족 유닛 상점 가산(+0.3, "레벨 5 빌드업 부족"), 이미 가진 기준 유닛은 가산 없음
- `tests/advisor/test_pinned_strict.py` 신규(12개): 고정 상점(자이라 고정 → 오른·알리스타·바루스·쉔 "고정 덱에 없음", 달빛 고정 → 살 수 있음), rescore_shop, `pin_other_rel` 0, 고정 아이템(고정 덱 아이템만·보조 없음 / 없으면 보관 문구, 고정 없으면 예전), 고정 판매(대안 덱 유닛 판매 후보 / 2스테이지 조용함), 성급 미상(사본 수·2성/3성 판정, Jev `star: null`, 자원 시그니처, 판매 최솟값·"이상"·"★?", 보드 "★?")
- 기존 테스트 수정: `test_unit_stage.test_board_plan_prefers_strong_now…`·`test_stage_boards.test_plan_fields_the_real_stage_board…` → `follow_buildup=false`로 예전 동작 고정 + 새 동작 단언 추가(`test_board_plan_follows_buildup_even_early`, 기본 모드에서 스테이지 보드 문구 없음), `test_advisor_units.test_two_star_bonus_only_when_units_known`(§17.7)
- 전체(`PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m pytest -o addopts="" -q`): 1523 passed, 4 skipped, 1 xfailed, 4 failed = 알려진 Windows 4건(api_key 힌트, setup 권한 상자, credentials 0600 두 건). 중간 실행에서 `tests/test_unit_track.py` 1건이 vision 동시 작업으로 실패했다가 마지막 실행에서는 통과했다.

### 17.11 남은 점 · 요청
- **app-integrator**
  - HUD 아이콘 줄에 `BoardPlan.missing`을 흐린 아이콘으로(또는 `reference_units` 전체를 보유/부족 표시로) 보여 주면 "레벨 5 빌드업" 줄을 아이콘으로 대신할 수 있다. `BoardPlanEntry.in_reference`로 임시 유닛을 구분 표시할 수 있다.
  - 보드 배치 글자 줄이 늘었다(기준 · 상점에서 구하세요 · 레벨 N). 아이콘 모드 기본 `lines_board` 6이면 기준·교체·상점·다음 레벨까지 보이고 판매 줄은 "… 외 N줄"로 접힐 수 있다 — 판매가 중요하면 `lines_board`를 7~8로.
  - `overlay._plan_color`: "상점에서 구하세요:"도 할 일(WARN) 색으로 칠할지 결정.
  - `hud_model.lineup_cells`는 ★2+만 별 표시한다. 성급 미상 "★?" 배지를 원하면 거기서 `e.star is None`을 쓴다.
  - 고정 해제 시 다시 대안 덱 경로 가중이 돌아온다. 오버레이 고정 표시 옆에 "고정 덱 전용 추천 중" 같은 안내가 있으면 사용자가 [보류] 이유를 바로 이해한다.
- **stats-researcher**: 빌드업 보드 행에 `top4`·`win_rate`가 있으면 기준 보드 동률 판정(지금 games → avg_place)을 개선할 수 있다. 1기짜리 레벨 5 보드 같은 잡음 행의 출처 확인 요청.
- **튜닝 후보**: `ref_top_boards`(3 — 늘리면 보유와 더 잘 맞는 보드를 고르지만 드문 보드로 흔들림), `reference_missing_bonus`(0.3), `ref_now_scale`(0.4), `star_unknown`(0.4 < keep 0.5 유지 필요).

### 17.12 QA 36 W3 후속: 저신뢰 프레임에서도 "상점에서 구하세요" = 상점 부족 가산 대상 — jev-strategist, 2026-09-25
- **문제**: 상점 부족 가산(`[shop] reference_missing_bonus`)은 `view.units_known`일 때만 계산했다. 보드 배치는 필드 신뢰도가 낮아도 `relaxed_view`(§14.2)로 계획을 세워 "상점에서 구하세요: …"를 낸다. 그래서 저신뢰 프레임에서는 목록에 있는 유닛이 가산 없이 [보류]로 남을 수 있었다.
- **수정(권장안 = 같은 목록 쓰기)**: 부족 목록은 목표 덱 문제지 인식 신뢰도 문제가 아니다. 그래서 가산을 끄지 않고 두 쪽이 같은 계산을 쓰게 했다.
  - `board_plan.plan_view(view)`: 보드 배치가 실제로 쓰는 View를 돌려준다(`units_known`이면 그대로, 아니면 `relaxed_view`, 계획을 못 세우면 None).
  - `board_plan.view_reference(view, comp, level, w)`: 그 View의 보유 집합과 보드 배치와 같은 레벨 규칙(레벨 모르면 보드 인원)으로 기준 보드를 만든다.
  - `scoring.shop_advice`: `view_reference`와 `plan_view`의 보유 집합으로 `ref_missing`을 만든다. `units_known` 조건은 없앴다. 기준 덱도 같다(`shop_target()` = 표시 1위·고정 덱 = 보드 배치 `scorer.shown[0]`).
- 문구는 바꾸지 않았다(`app/report.py`는 건드리지 않음). 요청 사항 없음.
- **테스트**: `test_buildup_reference.test_low_trust_frame_shop_bonus_matches_plan_missing`. 필드 신뢰도 0.5(tracked), 달빛 고정, 보드 알리스타·오른, 상점 자야·쉔·오른 → `plan.low_trust`이고, `plan.missing`의 상점 유닛(자야·쉔)은 모두 "레벨 5 빌드업 부족", 이미 가진 오른은 가산 없음.
- **전체**: 1530 passed, 4 skipped, 6 xfailed, 5 failed. 4건은 알려진 Windows 4건이다. 나머지 `tests/test_vision_units.py::test_report_lists_named_units`는 app-integrator가 작업 중인 `app/report.py:127`(유닛 줄의 " ★?", QA 36 W2) 때문이다. 기대 문자열이 "아칼리 (자리 미상)"이고 실제는 "아칼리 ★? (자리 미상)"이라 그쪽에서 테스트를 맞춰야 한다. 내 변경과는 무관하다.
