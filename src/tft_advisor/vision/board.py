...
BAR_MIN_W = 6 / 1080.0       # 이만큼 초록이 보이면 바 후보(성급 배지·테두리가 결정적 조건이다)
"""라이브 2(23 보고): 내 전략가 이름표("<내 소환사명> [4]")가 바로 뒤 유닛의 체력바를 **가려** 배지 옆 초록 8px만 보였다.
예전 하한 18px에서는 그 유닛을 통째로 놓쳤다. 바 가운데는 왼쪽 끝 + 만피 길이/2로 잡으므로 일부만 보여도 자리가 맞다.
전략가 이름표 바는 두께가 13px라(유닛 바 4~5px) `BAR_MAX_H`에서 걸러지고, 왼쪽에 성급 배지가 아니라 레벨 상자가 있다."""
BAR_MAX_H = 7 / 1080.0
